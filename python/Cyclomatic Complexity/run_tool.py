...
def run_coverage(repo: Path, coverage_args: list[str]) -> int:
    if not repo.is_dir():
        raise SystemExit(f"Repo path does not exist or is not a directory: {repo}")

    python = resolve_python()
    command = [python, "-m", "coverage", *coverage_args]
...
def get_repo_path(repo_arg: str | None) -> Path:
    ...
    if not repo_value:
        raise SystemExit(
            "Set DEFAULT_REPO_PATH in run_tool.py or pass --repo with your repository path."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Install or run coverage.py against a provided repo."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("install", help="Install coverage.py")

    run_parser = subparsers.add_parser("run", help="Run coverage.py")
    ...
    run_parser.add_argument(
        ...
        help="Arguments passed directly to coverage.py",
    )
...
def main() -> int:
    ...
    if args.command == "run":
        ...
        coverage_args = normalize_tool_args(args.coverage_args)
        return run_coverage(repo, coverage_args)
