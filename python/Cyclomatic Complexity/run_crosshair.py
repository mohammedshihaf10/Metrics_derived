...
def run_crosshair(repo: Path, tool_args: list[str]) -> int:
    if not repo.is_dir():
        raise SystemExit(f"Repo path does not exist or is not a directory: {repo}")

    python = resolve_python()
    command = [python, "-m", "crosshair", *tool_args]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Install or run CrossHair against a provided repo."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("install", help="Install CrossHair")

    run_parser = subparsers.add_parser("run", help="Run CrossHair")
    ...
    run_parser.add_argument(
        ...
        help="Arguments passed directly to CrossHair",
    )
