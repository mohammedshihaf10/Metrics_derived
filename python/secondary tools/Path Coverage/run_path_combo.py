...
def main() -> int:
    ap = argparse.ArgumentParser(description="Install or run coverage.py + ast paths against a provided repo.")
    ...
    return subprocess.run([resolve_python(), str(TOOL_DIR / "analyze_path_coverage.py"), str(repo), str(output), *normalize(args.tool_args)], cwd=str(ROOT)).returncode
