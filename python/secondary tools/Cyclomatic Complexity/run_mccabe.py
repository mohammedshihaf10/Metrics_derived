...
def run_tool(repo: Path, output_path: Path | None, tool_args: list[str]) -> int:
    ...
    result = subprocess.run([resolve_python(), "-m", "mccabe", *tool_args], cwd=str(repo), capture_output=True, text=True)
    if result.stdout:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Install or run mccabe against a provided repo.")
