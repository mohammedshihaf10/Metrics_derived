...
def run_tool(repo: Path, output_path: Path | None, tool_args: list[str]) -> int:
    ...
    build = subprocess.run([python, "-m", "wily", "build", "actionscicd", "tests"], cwd=str(repo), capture_output=True, text=True)
    if build.stdout:
    ...
    result = subprocess.run([python, "-m", "wily", *tool_args], cwd=str(repo), capture_output=True, text=True)
    if result.stdout:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Install or run wily against a provided repo.")
