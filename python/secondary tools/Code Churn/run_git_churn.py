...
TOOL_DIR = ROOT / "tools" / "git-churn"
DEFAULT_REPO_PATH = str((ROOT.parent / "github-actions-cicd-example").resolve())
DEFAULT_OUTPUT_PATH = str(ROOT / "git_churn_report.json")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Install or run git log --numstat against a provided repo.")
    ...
    return subprocess.run([resolve_python(), str(TOOL_DIR / "analyze_git_churn.py"), str(repo), str(output), *normalize(args.tool_args)], cwd=str(ROOT)).returncode
