...
def main() -> int:
    ap = argparse.ArgumentParser(description="Install or run pytest-cov against a provided repo.")
    ...
    if args.command == "install":
        return install_tool()
    return subprocess.run([resolve_python(), "-m", "pytest", *normalize(args.tool_args)], cwd=str(Path(args.repo or DEFAULT_REPO_PATH).resolve())).returncode
