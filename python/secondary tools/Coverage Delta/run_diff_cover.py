...
def run_tool(repo: Path, output_path: Path, tool_args: list[str]) -> int:
    ...
    return subprocess.run([resolve_python(), str(TOOL_DIR / "analyze_coverage_delta.py"), str(repo), str(output_path), *tool_args], cwd=str(ROOT)).returncode


def main() -> int:
    ap = argparse.ArgumentParser(description="Install or run coverage.py + git diff against a provided repo.")
