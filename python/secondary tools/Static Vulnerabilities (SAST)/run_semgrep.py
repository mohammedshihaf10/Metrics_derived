...
def run_tool(repo: Path, output_path: Path | None, tool_args: list[str]) -> int:
    ...
    python_path = Path(resolve_python())
    semgrep_exe = python_path.parent / ("semgrep.exe" if sys.platform.startswith("win") else "semgrep")
    cmd = [str(semgrep_exe), *tool_args] if semgrep_exe.is_file() else [str(python_path), "-m", "semgrep", *tool_args]
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Install or run semgrep against a provided repo.")
