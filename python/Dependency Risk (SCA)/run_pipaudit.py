...
TOOL_DIR = ROOT / "tools" / "pipaudit"
DEFAULT_REPO_PATH = str(ROOT.parent / "Cyclomatic Complexity" / "github-actions-cicd-example")
DEFAULT_OUTPUT_PATH = str(ROOT / "pipaudit_report" / "pipaudit.json")
...
def run_pipaudit(repo: Path, output_path: Path | None, audit_args: list[str]) -> int:
    if not repo.is_dir():
        raise SystemExit(f"Repo path does not exist or is not a directory: {repo}")

    python = resolve_python()
    command = [python, "-m", "pip_audit", *audit_args]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Install or run pip-audit against a provided repo.")
    subparsers = parser.add_subparsers(dest="command", required=True)
    subparsers.add_parser("install", help="Install pip-audit")

    run_parser = subparsers.add_parser("run", help="Run pip-audit")
    ...
    run_parser.add_argument(
        ...
        help="Arguments passed directly to pip-audit",
    )
    return parser
...
def main() -> int:
    ...
    if args.command == "install":
        return install_pipaudit()

    repo = Path(args.repo or DEFAULT_REPO_PATH).resolve()
    audit_args = normalize_tool_args(args.audit_args)
    output_path = Path(args.output).resolve() if args.output else None
    return run_pipaudit(repo, output_path, audit_args)
