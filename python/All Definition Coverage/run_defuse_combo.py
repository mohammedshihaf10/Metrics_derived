...
def analyze(repo: Path) -> dict[str, object]:
    ...
    defs_with_covered_use = {
        (p["scope"], p["name"], p["def_line"])
        for p in pairs
        if p["covered_pair"]
    }
    uncovered_defs = [
        d for d in defs
        if (d["scope"], d["name"], d["lineno"]) not in defs_with_covered_use
    ]
    ...
    return {
        ...
        "definitions_covered": sum(1 for d in defs if d["covered"]),
        "definitions_with_covered_use": len(defs_with_covered_use),
        "definitions": defs,
        "definition_use_pairs": pairs,
        "uncovered_definitions": uncovered_defs,
    }


def run_tool(repo: Path) -> int:
    result = analyze(repo)
    out = ROOT / "all_definition_report.json"
