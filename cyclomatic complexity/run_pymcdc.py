...
def run_pymcdc(repo: Path, tool_args: list[str]) -> int:
    if not repo.is_dir():
        raise SystemExit(f"Repo path does not exist or is not a directory: {repo}")

    python = resolve_python()
    command = [python, "-m", "pymcdc", *tool_args]
    env = os.environ.copy()
    env.setdefault("PYTHONIOENCODING", "utf-8")
    return subprocess.run(command, cwd=str(repo), env=env).returncode
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Install or run pymcdc against a provided repo."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("install", help="Install pymcdc")

    run_parser = subparsers.add_parser("run", help="Run pymcdc")
    ...
    run_parser.add_argument(
        ...
        help="Arguments passed directly to pymcdc",
    )
